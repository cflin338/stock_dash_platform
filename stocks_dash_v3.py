...
interval_dropdown_options = [{'label':'1s','value':'1s'},
                             {'label':'15m','value':'15m'},
                             {'label':'30m','value':'30m'},
                             {'label':'1h','value':'1h'},
                             {'label':'4h','value':'4h'},
...
app.layout = html.Div(children = [
    ...
    #-
    html.Div(children = [
        html.Div(children = [dcc.Input(id = 'ticker-input', value = '', type = 'text', 
                                       className = 'two columns'),
                             dcc.Input(id = 'rate-input', value = stock_options.get_current_risk_free_interest_rate(), 
                                       type = 'number', 
                                       className = 'two columns'),
                             html.Button(id = 'fill-ticker-table-info', n_clicks = 0, 
                                         children = 'Load Data', 
...
#                             style_table={'height': '300px', 'width': '500px','overflowY': 'auto'},
    ...
    html.Div(children = [
        dcc.Input(id = 'prev-date-select', value = datetime.datetime.today().strftime("%d/%m/%Y %H:%M:%S")[0:10], 
                  type = 'text',
                  className = 'two columns'),
        dcc.Dropdown(id = 'tmpid-dates', 
                     options = interval_dropdown_options, 
                     value = '1s',
...
def update_candlestick(click, date):
    #using 14, 30 simple MA, Keltner, closing prices
    ...
    if len(np.shape(df))>0:
        ...
        m1 = stock_options.moving_average(list(seq), 'simple',14)
        trace_14ma = go.Scatter(x = seq_x[14:], y = m1, mode = 'lines', showlegend = True, name = '14 simple MA')
        m2 = stock_options.moving_average(list(seq), 'simple',30)
        trace_30ma = go.Scatter(x = seq_x[30:], y = m2, mode = 'lines', showlegend = True, name = '30 simple MA')
        fig.add_trace(trace_14ma, secondary_y = False)
        fig.add_trace(trace_30ma, secondary_y = False)
        ...
        m3 = stock_options.moving_average(list(macd), 'simple',9)
